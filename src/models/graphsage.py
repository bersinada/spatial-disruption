"""Inductive GraphSAGE Baseline Model for Counterfactual Urban Accessibility (E02).

Implements:
  1. Inductive GraphSAGE node encoder operating over operational road network topology.
  2. Pairwise OD interaction head integrating node embeddings and pre-disruption routing.
  3. Multi-task heads: Post-disruption reachability classifier + conditional detour regressor.
  4. Exact metric calculation matching E01 protocol (ROC-AUC, PR-AUC, Brier score, MAE, Pearson r).
"""
...
class InductiveGraphSAGE(nn.Module):
    """Inductive GraphSAGE model for road network accessibility under disruption."""
    ...
    def __init__(
        ...
        self.detour_head = nn.Sequential(
            ...
            nn.Dropout(dropout),
            nn.Linear(hidden_dim, 1),
            nn.Softplus(),  # Smooth non-negative activation with non-zero gradients
        )
    ...
    def forward(
        ...
        """Forward pass for a single scenario graph.
        
        Args:
            x: Node feature tensor [N, D_node].
            edge_index: Operational corridor edges [2, E_op].
            origin_idx: Query origin node indices [B].
            dest_idx: Query destination node indices [B].
            pair_features: Pre-disruption query pairwise routing attributes [B, D_pair].
            
        Returns:
            Tuple of:
              - reach_logits: Predicted reachability logits [B].
              - detour_preds: Predicted relative detours [B].
        """
        # 1. Project node attributes
        h = self.node_proj(x)
        h_res = h
        
        # 2. GraphSAGE neighborhood aggregation over operational topology
        ...
        # 3. Formulate query OD pair representation
        h_o = h[origin_idx]
        h_d = h[dest_idx]
...
def evaluate_graphsage_scenarios(
    model: InductiveGraphSAGE,
    scenarios: List[Dict[str, Any]],
) -> Dict[str, Any]:
    """Evaluate GraphSAGE model across a list of scenarios."""
    model.eval()
